...
CYBERBOSS_DESIRE_DRIVEN=false
CYBERBOSS_DESIRE_COUPLING=false
CYBERBOSS_DESIRE_BASELINE_DRIFT=false
CYBERBOSS_HEARTBEAT_AUTONOMY=false
CYBERBOSS_DESIRE_SELF_DRIVE=false
...
- `CYBERBOSS_DESIRE_DRIVEN`, `CYBERBOSS_DESIRE_COUPLING`, `CYBERBOSS_DESIRE_BASELINE_DRIFT`, `CYBERBOSS_HEARTBEAT_AUTONOMY`, `CYBERBOSS_DESIRE_SELF_DRIVE`
  Optional desire-system gates. Leave them `false` by default, then turn them on one by one when validating proactive behavior.
...
- `npm run shared:open`
  Default attach path. Opens the bound shared thread in your terminal. When
  `CYBERBOSS_RUNTIME=claudecode`, this command connects through the local
  Claude IPC endpoint files stored under `CYBERBOSS_STATE_DIR`
  (`claudecode-runtime.json` and `claudecode-runtime.token`)
