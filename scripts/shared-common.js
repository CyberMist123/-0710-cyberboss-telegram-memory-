...
function findLiveAppServerPid() {
  try {
    const output = require("child_process").execSync(
      "ps -ef | grep 'codex app-server' | grep '--listen ws://127.0.0.1:' | grep -v grep",
      { encoding: "utf8" }
    );
...
function resolveBoundThread(workspaceRoot) {
  ...
  throw new Error(`no bound chat thread found for workspace: ${workspaceRoot}`);
}
