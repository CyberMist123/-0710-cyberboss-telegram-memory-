...
test("turn target binding also applies to an already-started pending run", async () => {
  const { sent, streamDelivery, bindingByThreadId } = createHarness();
  bindingByThreadId.set("thread-pending", { bindingKey: "binding-pending" });
  streamDelivery.setReplyTarget("binding-pending", {
    userId: "user-pending",
    contextToken: "ctx-weixin",
    provider: "weixin",
  });
  ...
  await streamDelivery.handleRuntimeEvent({
    type: "runtime.turn.started",
    payload: { threadId: "thread-pending", turnId: "" },
  });
  ...
  await streamDelivery.handleRuntimeEvent({
    type: "runtime.turn.completed",
    payload: { threadId: "thread-pending", turnId: "turn-pending" },
  });
...
test("plain weixin reply can be transformed before sending", async () => {
