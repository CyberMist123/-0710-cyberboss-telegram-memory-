...
test("desire checkin prompt keeps the 8-drive contract + trigger body when minimal loop gate is off", () => {
  // 文案已外置到 prompts/desire_checkin.md（可编辑、即时生效），内置文本只是
  // fail-open 回退。所以这里不再逐字钉死文案，改为守护不可变的契约：内部触发
  // 声明、八维完整维度、JSON 输出结构、时间戳与 Trigger body 的拼接。
  ...
  try {
    ...
    assert.ok(text.startsWith("[本地时间 2026-07-12 10:00:00]"), "keeps localized timestamp header");
    assert.match(text, /SYSTEM ACTION MODE: internal trigger, not user chat\./);
    assert.match(text, /System trigger type: desire_checkin\./);
    assert.match(text, /依恋\(attachment\)、好奇\(curiosity\)、沉思\(reflection\)、责任\(duty\)、社交\(social\)、疲惫\(fatigue\)、性欲\(libido\)、压力\(stress\)/, "keeps all 8 drives");
    assert.match(text, /drives 必须有全部 8 个 key/);
    assert.match(text, /"action":"silent","desire_state"/, "keeps the silent JSON shape");
    assert.ok(text.trimEnd().endsWith("Trigger:\n测试 checkin"), "appends the trigger body");
  } finally {
