...
test("codex MCP config auto-approves cyberboss tools", () => {
  ...
  assert.match(
    args.join("\n"),
    /mcp_servers\.cyberboss_tools\.tools\.cyberboss_reminder\.approval_mode="auto"/
  );
  assert.match(
    args.join("\n"),
...
test("codex MCP config includes external music MCP when configured", () => {
  const originalCommand = process.env.CYBERBOSS_MUSIC_MCP_COMMAND;
  const originalName = process.env.CYBERBOSS_MUSIC_MCP_NAME;
  const originalArgs = process.env.CYBERBOSS_MUSIC_MCP_ARGS;
  process.env.CYBERBOSS_MUSIC_MCP_COMMAND = "/opt/netease-music-mcp";
  process.env.CYBERBOSS_MUSIC_MCP_NAME = "netease_music_mcp";
  process.env.CYBERBOSS_MUSIC_MCP_ARGS = "--stdio --foo";
  ...
  if (originalCommand === undefined) {
    ...
    process.env.CYBERBOSS_MUSIC_MCP_COMMAND = originalCommand;
  }
  if (originalName === undefined) {
    delete process.env.CYBERBOSS_MUSIC_MCP_NAME;
  } else {
    process.env.CYBERBOSS_MUSIC_MCP_NAME = originalName;
  }
  if (originalArgs === undefined) {
    delete process.env.CYBERBOSS_MUSIC_MCP_ARGS;
  } else {
    process.env.CYBERBOSS_MUSIC_MCP_ARGS = originalArgs;
  }

  assert.match(
    args.join("\n"),
    /mcp_servers\.netease_music_mcp\.command="\/opt\/netease-music-mcp"/
  );
  assert.match(
    args.join("\n"),
    /mcp_servers\.netease_music_mcp\.args=\["--stdio","--foo"\]/
  );
});

test("codex MCP config includes extra MCP servers from JSON config", () => {
  const originalExtra = process.env.CYBERBOSS_EXTRA_MCP_SERVERS;
  process.env.CYBERBOSS_EXTRA_MCP_SERVERS = JSON.stringify([{
    name: "ruota_fortuna_mcp",
    command: "/usr/bin/node",
    args: ["/srv/ruota/server.js"],
  }]);
  ...
  if (originalExtra === undefined) {
    ...
    process.env.CYBERBOSS_EXTRA_MCP_SERVERS = originalExtra;
  }

  assert.match(
    args.join("\n"),
    /mcp_servers\.ruota_fortuna_mcp\.command="\/usr\/bin\/node"/
  );
  assert.match(
    args.join("\n"),
    /mcp_servers\.ruota_fortuna_mcp\.args=\["\/srv\/ruota\/server\.js"\]/
  );
});
  ...
  const event = mapCodexMessageToRuntimeEvent({
    ...
    params: {
      ...
      _meta: {
        ...
        tool_description: "Manage reminders in Cyberboss. Input: { command: 'create' | 'update' | 'delete', reminderId?: string, text?: string, delayMinutes?: integer, dueAt?: string }",
        tool_params_display: [
          { name: "delayMinutes", display_name: "delayMinutes", value: 5 },
          { name: "text", display_name: "text", value: "hello" },
        ],
      },
      message: 'Allow the cyberboss_tools MCP server to run tool "cyberboss_reminder"?',
      requestedSchema: {
        type: "object",
        properties: {},
      },
...
test("handleRuntimeEvent auto-approves project-native Codex MCP elicitation approvals", async () => {
  ...
  await CyberbossApp.prototype.handleRuntimeEvent.call(appLike, {
    ...
    payload: {
      ...
      commandTokens: ["mcp_tool", "cyberboss_tools", "cyberboss_reminder"],
      responseTemplate: {
        responseByCommand: {
          yes: {
            action: "accept",
