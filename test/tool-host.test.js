  ...
  return new ProjectToolHost({
    services: {
      ...
      reminder: {
        ...
        async update(args) {
          return { id: args.reminderId || "reminder-1", ...args };
        },
        async delete(args) {
          return { id: args.reminderId || "reminder-1", ...args };
        },
...
test("tool host exposes reminder command tool", async () => {
  const host = createHost();
  const createResult = await host.invokeTool("cyberboss_reminder", {
    command: "create",
    text: "ping me",
    delayMinutes: 30,
  }, {});
  const updateResult = await host.invokeTool("cyberboss_reminder", {
    command: "update",
    reminderId: "reminder-1",
    text: "updated text",
  }, {});
  const deleteResult = await host.invokeTool("cyberboss_reminder", {
    command: "delete",
    reminderId: "reminder-1",
  }, {});

  assert.equal(createResult.text, "Reminder queued: reminder-1");
  assert.equal(updateResult.text, "Reminder updated: reminder-1");
  assert.equal(deleteResult.text, "Reminder deleted: reminder-1");
});
