...
| 子代理结果胶囊化 / Route 1 task-session core（T09） | `ORPHAN` | `COVERED` | `BLOCKING` | `NOT_WIRED` | D14 胶囊契约与独立 `runTaskSession()` core 已实现：worker 使用独立 work profile / task lane / native session slot，结果经 parse→D14 validate→编排者 verify 后只向调用方返回胶囊与短状态；只读查询复用 D14 v1 空 files/tests/null commit 形状，worker 受 `work-memory-readonly@1` ceiling 约束，强中断当前仅有原子步骤边界 latch。挂 `CYBERBOSS_CLAUDE_ROUTE1_TASK_SESSION_ENABLED` 且默认关闭，测试进 `test:route-lanes` + `test:orchestration`（阻塞主 CI）；**主 Chat、origin window 投递、自动派活与完整强中断仍未接**（归 T10），故目标通路仍为 `ORPHAN` / `NOT_WIRED` |
