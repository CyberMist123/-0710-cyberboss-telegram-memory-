...
| 记忆服务层（validator / resolver） | `WIRED` | `PARTIAL` | `BLOCKING` | `WIRED` | **6 个**测试文件已接 `test:memory-services`（note-service / resolver / service-cleanup / service-formal / validator / command-router）。**extractor 一侧已按 `DECISIONS.md` D23 整条退役** —— 正则分桶抽取器与 post-response 自动写入 pipeline 已删除，系统不再替主体 AI 决定该记什么；退役由 `test/phase1-offline-config.test.js` 的守卫钉住。**仍无 CI 信号的**：`memory-command` / `memory-seven-day-cleanup` / `memory-validator-integration` 三个已由 #91 修绿但尚未接线（归后续接线批次）。故测试记 `PARTIAL` 而非 `COVERED`。本行剩余的 validator / resolver 仍是 memory_context 读取通路与 `/memory` 命令的实现方，读取侧改造归 #42（见 D21）|
