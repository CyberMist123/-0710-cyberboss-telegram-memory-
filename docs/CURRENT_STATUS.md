...
| G3 Route 2 静态 gate（T07） | `WIRED` | `COVERED` | `BLOCKING` | `DISABLED` | 挂 `CYBERBOSS_ROUTE2_GATE_ENABLED` 且默认关闭；A/B 成本门与结构硬门只产出“留 Route 2 / 转 Route 1”，不削减 chat 能力、不复用 G3 fail-closed ceiling。状态绑定既有 session slot 并复用 window override 指纹；真实 `tool.use`、逐工具 `max_result_bytes` 与 server 截断、成本 Context Trace 已接通，null/不可截断返回体转 Route 1；测试进 `test:route-lanes`、`test:catalog-metering`、`test:phase2`（阻塞主 CI），未做真机启用 |
