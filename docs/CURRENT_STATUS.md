...
| MCP 工具分组隐藏（省 schema token） | `ABSENT` | `NONE` | `NONE` | `NOT_WIRED` | D25-A 已解除 `DEFERRED` —— 工具面目录化获批（目录化是省 token 手段，不是授权边界，见 `DECISIONS.md` D25）。T01 计量基线已进仓：`scripts/audit/` 只读计量脚本 + `test/fixtures/catalog-metering-baseline.json` 基线（49 条、schema 15,810 chars）+ `test:catalog-metering` 阻塞主 CI；目录化本体仍未开工，故代码列仍 `ABSENT` |
