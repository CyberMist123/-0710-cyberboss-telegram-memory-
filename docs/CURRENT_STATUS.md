...
| 慢层注入面 E1（agreements / portrait / wandering） | `WIRED` | `COVERED` | `BLOCKING` | `VERIFIED` | D41：开窗小预算缝入，三项独立开关（生产已开），逐项 admit、超限整项跳过、缺失静默跳过（fail-open 只读）。2026-08-09 真机留证 portrait 注入 467 字；agreements/wandering 长出正文后自动生效；**2026-08-22 开机注入温度修复（`9a7fc35`+`d14676c`，真机 22:40 上线，Owner 验过）**：① 慢层总预算上限 4000→12000（生产 env 配 8000）——亲笔画像 2630 + agreements 4261 此前任一超限即整项无声跳过，开机只剩空年表与灰名单；② CURRENT_STATE 按 desire-state.json `updatedAt` 算时效，>2h 前缀「（N 小时前的姿态，不是现在）」，只标不丢（Owner 裁定），驱力分数行不再注入，trace 带 `age_hours`；③ persona 信改版（人在前、工具附录、加「她」与「醒来先读 ai_self_notes」指路，旧版 .bak-20260822）。仍欠：`reentry.md` 自 8-9 空文件无人写、ai_self_notes 不在开机链上只靠自读。 |
