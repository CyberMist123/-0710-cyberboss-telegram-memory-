...
| 子代理结果胶囊化 / Route 1 task-session core（T09） | `ORPHAN` | `COVERED` | `BLOCKING` | `NOT_WIRED` | D14 胶囊契约与独立 `runTaskSession()` core 已实现；T10-A 在新增 `CYBERBOSS_ROUTE1_CHAT_DISPATCH_ENABLED`（默认关闭且要求 T09 开关同时开启）后补齐离线预防边界：worker 由编排器供给独立 git worktree 以隔离锁域，建卡时 fail-closed 拒绝与活 memory/continuity/state/settings-secrets/profile-configRoot 重叠的 workspace/allowed_paths，并把这些绝对根并入 forbidden_paths；work profile 加 Write/Edit deny，完成后由编排器自跑 git 取证再验胶囊。保护对象仅为活运行时数据，不按名称封锁 `src/` 或 `extensions/relationship-memory/` 源码；无 OS 沙箱，Bash 绝对路径逃逸仍是明确残余风险。测试进 `test:route-lanes` + `test:orchestration`（阻塞主 CI）；**主 Chat、origin window 投递、自动派活、双上限与完整强中断仍未接**（归 T10-B/T10-C），故目标通路仍为 `ORPHAN` / `NOT_WIRED` |
