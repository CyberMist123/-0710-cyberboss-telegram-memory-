...
Route 2 单次 capability lease 随 mutable override restart+resume 生效，并在完成、失败、TTL、cancel、强中断或 restart 时回收；回收后目录仍可浏览，但 lease 成员的 schema loader 与 call gate 均以 `capability_lease_expired` 拒绝。
...
`/pause_heartbeat` 与 `/continue_heartbeat` 只控制窗口聊天之外的自主心跳：Desire 小时 tick、定时签到、closeout/liveness 新调度，以及 `desire_checkin` / `checkin` / `liveness_alert` 三种已排队消息的投递。窗口聊天、用户 reminder、位置事件与其他显式 system message 不在暂停面内。正在运行的回合不取消，下一次 tick 才读取暂停态。
