...
### 报错 3：`Claude IPC endpoint not found`
...
- 所以 `shared:open` 试图连接 Claude 的本地 IPC 端点
...
如果你本来就是要跑 `claudecode`，那排查重点不是 `.sock` 文件，而是先确认 bridge 已经启动，并且 `CYBERBOSS_STATE_DIR` 下已经生成：

- `claudecode-runtime.json`
- `claudecode-runtime.token`

只要这两个文件存在，`npm run shared:open` 就可以接入本地 Claude bridge。
