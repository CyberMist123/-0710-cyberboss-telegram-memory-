...
5. 以 `IMPLEMENTATION_STATUS.md` 的未收敛项为索引，逐项在源码和真实运行现场验证，不把 README 当 bug 清单。
...
- 文档冲突时按优先级执行，并把冲突记录进 `IMPLEMENTATION_STATUS.md` 等用户裁决。
...
只以 `IMPLEMENTATION_STATUS.md` 的验收标准为准。更新该文档时只写：新增的真实能力、状态变化、实际 bug、范围变化。禁止写“理论上完成”或堆叠思考过程。
