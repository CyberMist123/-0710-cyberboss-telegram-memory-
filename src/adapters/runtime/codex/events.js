...
function mapCodexMessageToRuntimeEvent(message) {
  ...
  if (isThinkingMethod(method)) {
    const text = extractThinkingText(params);
    if (text) {
      return {
  ...
  if (method === "item/completed" && normalizeString(params?.item?.type || params?.itemType).toLowerCase() === "agentmessage") {
    const text = extractAssistantText(params);
    return {
  ...
  if (method === "item/completed") {
    const itemType = normalizeString(params?.item?.type || params?.itemType).toLowerCase();
    if (itemType === "reasoning") {
      return {
        ...
        payload: {
          threadId,
          turnId,
          itemId: normalizeString(params?.item?.id || params?.itemId),
          text: extractThinkingText(params) || extractAssistantText(params),
          sourceMethod: method,
          itemType,
        },
      };
    ...
    return {
      ...
      payload: {
        ...
        itemId: normalizeString(params?.item?.id || params?.itemId),
        itemType,
        text: extractAssistantText(params) || extractThinkingText(params),
      },
    };
  }
  ...
  if (method === "item/started") {
    return {
      ...
      payload: {
        ...
        turnId,
        itemId: normalizeString(params?.item?.id || params?.itemId),
        itemType: normalizeString(params?.item?.type || params?.itemType).toLowerCase(),
        text: extractThinkingText(params),
      },
    };
  }
...
function isThinkingMethod(method) {
  const lower = normalizeString(method).toLowerCase();
  return lower.includes("reason") || lower.includes("thinking") || lower.includes("analysis");
}

function extractThinkingText(params) {
  const candidates = [
    params?.text,
    params?.delta,
    params?.content,
    ...
    params?.item?.delta,
    params?.item?.summary,
    params?.item?.summaryText,
    params?.item?.summary_text,
    params?.summary,
  ];
      ...
      const joined = value
        .map((entry) => {
          ...
          if (entry && typeof entry === "object") {
            return String(entry.text || entry.content || entry.summary || entry.value || "").trim();
          }
          return "";
        })
        .filter(Boolean)
