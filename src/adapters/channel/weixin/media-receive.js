...
async function persistSingleAttachment({ attachment, stateDir, cdnBaseUrl, messageId, receivedAt }) {
  ...
  const compressed = await maybeCompressImageAttachment({
    attachment,
    plaintext,
  ...
  const absolutePath = await writeUniqueFile(targetDir, fileName, compressed.bytes);
...
async function maybeCompressImageAttachment({ attachment, plaintext, contentType }) {
  if (!isImageAttachment({ kind: attachment?.kind, contentType, fileName: attachment?.fileName })) {
    return { bytes: plaintext, contentType };
  }
  ...
  try {
    ...
    const shouldResize = Number.isFinite(longestEdge) && longestEdge > 1344;
    const targetWidth = shouldResize && width >= height ? 1344 : Math.max(1, Math.round(width * (1344 / longestEdge)));
    const targetHeight = shouldResize && height > width ? 1344 : Math.max(1, Math.round(height * (1344 / longestEdge)));
...
function buildTargetFileName({ attachment, plaintext, contentType, messageId }) {
  ...
  if (sourceName) {
    ...
    if (existingExt) {
      if (isImageAttachment({ kind: attachment?.kind, contentType, fileName: sourceName })) {
        return `${path.basename(sourceName, existingExt)}.jpg`;
      }
      return sourceName;
    }
...
function inferExtension({ contentType, plaintext, kind }) {
  if (normalizeContentType(contentType) === "image/jpeg") {
    return ".jpg";
  }
