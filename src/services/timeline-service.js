...
class TimelineService {
  ...
  async write({
    ...
    const normalizedEvents = Array.isArray(events)
      ? tagTimelineEvents(events, this.config.sourceLabel)
      : undefined;
    if (date) {
      args.push("--date", date);
    }
    ...
    } else if (Array.isArray(normalizedEvents)) {
      args.push("--events-json", JSON.stringify({ events: normalizedEvents }));
    } else if (eventsJson) {
...
function tagTimelineEvents(events, sourceLabel = "") {
  const normalizedSourceLabel = normalizeText(sourceLabel);
  if (!normalizedSourceLabel) {
    return events;
  }
  const sourceTag = `source:${normalizedSourceLabel}`;
  const sourceNoteLine = `source: ${normalizedSourceLabel}`;
  return events.map((event) => {
    ...
    const nextNote = note
      ? (note.includes(sourceNoteLine) ? note : `${note}\n${sourceNoteLine}`)
      : sourceNoteLine;
    return {
      ...event,
      note: nextNote,
...
function resolveScreenshotOutputFile(config, outputFile = "") {
  ...
  const shotsDir = path.join(normalizeText(config.timelineStateDir) || config.stateDir, "timeline", "shots");
  const now = new Date();
  const stamp = [
    now.getFullYear(),
