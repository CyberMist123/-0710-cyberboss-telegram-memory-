...
class ReminderService {
  ...
  async create({
    ...
    const contextToken = resolveReminderContextToken({
      config: this.config,
      accountId: account.accountId,
      senderId,
    });
  ...
  async update({
    ...
    if (!normalizedReminderId) {
      throw new Error("Reminder id cannot be empty.");
    }
    ...
    if (delay !== "" || delayMinutes !== undefined || at !== "" || dueAt !== "") {
      const dueAtMs = resolveDueAtMs({ delay, delayMinutes, at, dueAt });
      if (!Number.isFinite(dueAtMs) || dueAtMs <= Date.now()) {
        throw new Error("Missing a valid time. Use delayMinutes or dueAt like 2026-04-07T21:30+08:00.");
      }
    ...
    if (normalizeText(userId)) {
      ...
      patch.senderId = senderId;
      patch.contextToken = contextToken;
    }

    const reminder = this.queue.updateById(normalizedReminderId, {
      ...patch,
      updatedAt: new Date().toISOString(),
    });
    if (!reminder) {
      throw new Error(`Cannot find reminder ${normalizedReminderId}.`);
    }
    return reminder;
  }
  ...
  async delete({ reminderId = "" } = {}) {
    ...
    const removed = this.queue.deleteById(normalizedReminderId);
    if (!removed) {
      throw new Error(`Cannot find reminder ${normalizedReminderId}.`);
    }
    return removed;
...
function resolveReminderContextToken({ config, accountId, senderId }) {
