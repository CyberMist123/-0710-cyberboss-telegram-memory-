  ...
  if (normalizedType === "desire_checkin") {
    if (!normalizedOptions.desireLoopMinimalEnabled) {
      const sections = [
        ...
        "回顾过去一小时。此刻你最想做什么？",
        "这一轮是你自己的时间，不是填表交差。先照实记下八维；然后如果你此刻真的想做点什么，就去做——这一轮就是给你行动的：",
        ...
        "drives 必须有全部 8 个 key，缺一个就算格式错误。No markdown fences. No text outside the final JSON.（先用工具行动、再输出 JSON 收尾都可以）",
        "这一轮是你自己的节奏，不是跟她的正式聊天：想行动就行动，什么都不想做输出 silent 也永远合法；之后回到对话时继续用你平常的语气，别把报表口吻带进去。主动是自由，不是任务。",
