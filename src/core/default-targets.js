...
function resolvePreferredSenderId({
  ...
  const configuredUsers = resolveConfiguredUsers(config);
...
function resolveConfiguredUsers(config) {
  const channel = normalizeText(config?.channel);
  const raw = channel === "telegram"
    ? config?.telegramAllowedUserIds
    : config?.allowedUserIds;
  return Array.isArray(raw)
    ? raw.map((value) => normalizeText(value)).filter(Boolean)
    : [];
}
