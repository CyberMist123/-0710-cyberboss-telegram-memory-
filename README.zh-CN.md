...
- `npm run shared:open`
  默认接管方式。跨平台接入当前微信绑定的那条共享线程。当
  `CYBERBOSS_RUNTIME=claudecode` 时，这个命令会通过
  `CYBERBOSS_STATE_DIR` 下的 Claude 本地 IPC 端点文件
  `claudecode-runtime.json` 和 `claudecode-runtime.token` 连接到运行中的 bridge
...
Cyberboss 现在支持把外部音乐 MCP 作为额外 project tool 挂进来。你可以把 `luuu-h/netease-music-mcp` 这类 server 通过环境变量注入给 Cyberboss，然后让 Codex / Claude Code 直接调用它。

推荐配置示例：

```dotenv
CYBERBOSS_MUSIC_MCP_COMMAND=/path/to/netease-music-mcp
CYBERBOSS_MUSIC_MCP_ARGS=--stdio
CYBERBOSS_MUSIC_MCP_NAME=netease_music_mcp
```

说明：

- `CYBERBOSS_MUSIC_MCP_COMMAND`
  音乐 MCP 的启动命令或可执行文件路径。
- `CYBERBOSS_MUSIC_MCP_ARGS`
  传给音乐 MCP 的参数，按空格分隔。
- `CYBERBOSS_MUSIC_MCP_NAME`
  MCP server 名称，默认是 `netease_music_mcp`。

如果你使用 Claude Code，Cyberboss 会把这个 server 写入当前 workspace 的 `.mcp.json`。如果你使用 Codex，Cyberboss 会把它作为额外 `mcp_servers` 注入到启动参数里。
